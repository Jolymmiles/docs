...
Поместите свой JPEG/PNG в `assets/menu.jpg`, добавьте к `volumes` сервиса `bot` строку `- ./assets:/assets:ro` и задайте `MENU_PHOTO_PATH=/assets/menu.jpg` в `.env`. В актуальном Compose из репозитория mount уже присутствует. Без этой переменной бот работает с текстовым меню.
