...
Для бакетов Selectel с virtual-hosted адресацией установите `S3_FORCE_PATH_STYLE=false`. Иначе клиент сформирует path-style URL, и имя бакета может попасть в ключ объекта.
...
| `S3_FORCE_PATH_STYLE` | Адресация бакета: path-style (`endpoint/bucket/key`) включена по умолчанию; только значение `false` переключает на virtual-hosted style (`bucket.endpoint/key`) |
:::
